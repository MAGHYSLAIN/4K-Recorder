...
namespace MText
{
    ...
    public class Mtext_UI_InputField : MonoBehaviour //Damm. Noticed MText was written as Mtext but can't change it now without causing users to lose their serialized fields
    {
        ...
        private void UnselectedVisual()
        {
            ...
            {
                Material textMat = string.IsNullOrEmpty(_text) ? placeHolderTextMat : applyNormalStyle.Item2.normalItemFontMaterial;
                UpdateMaterials(textMat, applyNormalStyle.Item2.normalItemBackgroundMaterial);
            }
            //apply self mat
            else
            {
        ...
        public void DisableVisual()
        {
            ...
            if (applySelectedItemMaterial.Item1)
            {
                Material textMat = string.IsNullOrEmpty(_text) ? placeHolderTextMat : applySelectedItemMaterial.Item2.disabledItemFontMaterial;
                UpdateMaterials(textMat, applySelectedItemMaterial.Item2.disabledItemBackgroundMaterial);
            }
            //apply self mat
            else
            {
