...
namespace MText
{
    ...
    public class Modular3DText : MonoBehaviour
    {
        ...
        //    }
        //}
        //public bool DoesTextNeedToBeUpdated()
        //{
        //    bool createChild = ShouldItCreateChild();
        ...
        //        }
        //    }
        //    //continues if text isn't empty

        //    if (createChild)
        //    {
        //        //should create child but has no child
        //        if (characterObjectList.Count == 0)
        //            return true;

        //        if (characterObjectList[0].GetComponent<Renderer>().sharedMaterial != Material)
        //        {
        //            return true;
        //        }
        ...
        //    else
        //    {
        //        //should have meshfilter
        //        if (!GetComponent<MeshFilter>())
        //            return true;
        //        //should have mesh but no mesh
        //        if (GetComponent<MeshFilter>().sharedMesh == null)
        //            return true;

        //        if (!GetComponent<Renderer>())
        //            return true;
        //        if (!GetComponent<Renderer>().sharedMaterial != Material)
        //            return true;
        //    }
        ...
        {
            //Debug.Log("CheckLeftOversInEditorAndCleanUp running");

            for (int i = _allcharacterObjectList.Count - 1; i >= 0; i--)
            {
                //Debug.Log("Checking character object: ", _allcharacterObjectList[i]);

                if (_allcharacterObjectList[i] == null)
                {
                    //Debug.Log("Character removed from list for being null.");
                ...
                {
                    //Debug.Log("Character being deleted added to delay call: " + _allcharacterObjectList[i]);
        ...
        {
            //Debug.Log("Character being deleted because it isn't contained in character object list.");
            try
            {
        ...
        private void CombineMeshes()
        {
            ...
            Vector3 oldScale = transform.localScale; //changed from local scale

            transform.rotation = Quaternion.identity;
            transform.position = Vector3.zero;

            //Lossy scale is causing issues if the scale is skewed
            Vector3 lossyScale = transform.lossyScale;
            if (lossyScale != transform.localScale)
            {
                lossyScale = new Vector3((1f / lossyScale.x) * oldScale.x, (1f / lossyScale.y) * oldScale.y, (1f / lossyScale.z) * oldScale.z); //convert it into vector3.one world scale
                transform.localScale = lossyScale;
                //Debug.Log("Using lossy" + lossyScale);
            }
            else
            {
        ...
        public bool DoesStyleInheritFromAParent()
        {
            if (transform.parent)
            {
                if (transform.parent.gameObject.GetComponent<MText_UI_Button>())
                {
                    if (transform.parent.gameObject.GetComponent<MText_UI_Button>().text == this)
                        return true;
                }
            }
